...
def place_bid(auction_id):
    ...
    if bid_amount > auction['current_bid']:
        # ── Get car name for notification messages ──
        car_row = conn.execute("SELECT name FROM cars WHERE id=?", (auction['car_id'],)).fetchone()
        car_name = car_row['name'] if car_row else f"Auction #{auction_id}"

        # ── Remember previous highest bidder before updating ──
        prev_highest_bidder_id = auction['highest_bidder_id']
        prev_highest_bid = auction['current_bid']

        # ── Update auction ──
        bid_status = get_bid_status(bid_amount, auction['start_price'])
        conn.execute(
            "UPDATE auctions SET current_bid=?, highest_bidder_id=?, bid_count=bid_count+1 WHERE id=?",
            (bid_amount, dealer_id, auction_id)
        )
        conn.execute(
            "INSERT INTO bids (auction_id, dealer_id, bid_amount, bid_status) VALUES (?,?,?,?)",
            (auction_id, dealer_id, bid_amount, bid_status)
        )

        # ── Notification 1: New highest bidder — you are winning ──
        conn.execute(
            "INSERT INTO notifications (dealer_id, message) VALUES (?, ?)",
            (dealer_id,
             f'🏆 Aap ab HIGHEST BIDDER hain! Aapki ₹{bid_amount:,.0f} ki bid "{car_name}" mein sabse aage hai. Keep watching!')
        )

        # ── Notification 2: Previous highest bidder — you are outbid ──
        if prev_highest_bidder_id and prev_highest_bidder_id != dealer_id:
            conn.execute(
                "INSERT INTO notifications (dealer_id, message) VALUES (?, ?)",
                (prev_highest_bidder_id,
                 f'🔴 Aapki ₹{prev_highest_bid:,.0f} ki bid "{car_name}" par OUTBID ho gayi! Ab highest bid ₹{bid_amount:,.0f} hai. Abhi counter-bid karo!')
            )

        # ── Notification 3: All OTHER dealers who bid on this auction (not the new top bidder, not prev highest) ──
        other_bidders = conn.execute(
            """SELECT DISTINCT dealer_id FROM bids
               WHERE auction_id=? AND dealer_id != ? AND dealer_id != ?""",
            (auction_id, dealer_id, prev_highest_bidder_id or -1)
        ).fetchall()
        for row in other_bidders:
            conn.execute(
                "INSERT INTO notifications (dealer_id, message) VALUES (?, ?)",
                (row['dealer_id'],
                 f'⚡ "{car_name}" auction mein nayi high bid ₹{bid_amount:,.0f} aayi hai! Aap peeche hain — bid badhao!')
            )

        conn.commit()
        flash(f'🏆 Bid ₹{bid_amount:,.0f} placed! Aap abhi HIGHEST bidder hain.', 'success')
    else:
...
# ─── API: Unread Notification Count (for live badge polling) ───
...
def api_unread_notifications():
    ...
    conn = get_db()
    count = conn.execute(
        "SELECT COUNT(*) FROM notifications WHERE dealer_id=? AND is_read=0", (dealer_id,)
    ).fetchone()[0]
    latest = conn.execute(
        "SELECT message, created_at FROM notifications WHERE dealer_id=? AND is_read=0 ORDER BY created_at DESC LIMIT 5",
        (dealer_id,)
    ).fetchall()
    conn.close()
    return jsonify({'unread_count': count, 'latest': [dict(r) for r in latest]})
...
def mark_notifications_read():
    dealer_id = session['dealer_id']
    conn = get_db()
    conn.execute("UPDATE notifications SET is_read=1 WHERE dealer_id=?", (dealer_id,))
    conn.commit()
    conn.close()
